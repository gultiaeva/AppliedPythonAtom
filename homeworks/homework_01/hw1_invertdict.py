        ...
        if isinstance(v, UNHASHABLE):
            ...
            for item in input_dict[k]:
                if item in output_dict:
                    tmp = [output_dict[item]] if not isinstance(output_dict[item], list) else output_dict[item]
                    tmp.append(k)
            ...
            if v in output_dict:
                tmp = [output_dict[v]] if not isinstance(output_dict[v], list) else output_dict[v]
                tmp.append(k)
                output_dict[v] = tmp
            else:
                output_dict[v] = k
